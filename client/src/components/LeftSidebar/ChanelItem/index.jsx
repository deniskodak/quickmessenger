...
const ChanelItem = ({lastMessage, lastMessageTime, name, img, id, background}) => {
	...
	return (
		<li className={`flex w-full items-center py-2 cursor-pointer rounded-lg px-2 ${backgroundClasses}`} onClick={handleClick}>
			<Avatar name={name} img={img} background={background}/>
            ...
            <p className={`mr-2 font-semibold ${textColorClasses.name} ${OVERFLOW_CLASSES}`}>{name}</p>
